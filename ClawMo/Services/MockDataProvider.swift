...
enum MockDataProvider {
    ...
    static func load(into store: AppStore) {
        ...
        store.agentList = mockAgents.enumerated().map { i, m in
            AgentInfo(id: "agent-\(i)", name: m.0, avatar: m.1, color: "#94a3b8")
        }
        ...
        store.conversations = [
            Conversation(id: "user:agent-0", sessionKey: sk0, sessionKeys: [sk0], agentId: "agent-0",
                         displayName: "小南", avatar: "star.fill", color: "#94a3b8", kind: .user,
            ...
            Conversation(id: "user:agent-2", sessionKey: sk2, sessionKeys: [sk2], agentId: "agent-2",
                         displayName: "码匠", avatar: "laptopcomputer", color: "#94a3b8", kind: .user,
            ...
            Conversation(id: "user:agent-3", sessionKey: sk3, sessionKeys: [sk3], agentId: "agent-3",
                         displayName: "探针", avatar: "magnifyingglass", color: "#94a3b8", kind: .user,
                         ...
                         lastTimestamp: now.addingTimeInterval(-600), historyLoaded: true, fullyLoaded: true),
            {
                var c = Conversation(id: "a2a:agent-0:agent-2", sessionKey: a2a_02, sessionKeys: [a2a_02], agentId: "agent-2",
                             displayName: "小南", avatar: "star.fill", color: "#94a3b8", kind: .a2a,
        ...
        // File messages
        var fileMsg1 = ChatMessage(id: "file-1", sessionKey: sk0, agentId: "agent-0", role: .user,
                    text: "[文件: 需求文档v3.pdf]", timestamp: now.addingTimeInterval(-150), runId: nil)
        fileMsg1.fileSize = 2_358_000
        fileMsg1.localImageData = "这是一个 PDF 文件的模拟数据".data(using: .utf8)
        allMessages.append(fileMsg1)

        allMessages.append(ChatMessage(id: "file-1-reply", sessionKey: sk0, agentId: "agent-0", role: .agent,
                    text: "收到「文件: 需求文档v3.pdf」。小南已处理完毕，一切正常。",
                    timestamp: now.addingTimeInterval(-140), runId: nil))

        var fileMsg2 = ChatMessage(id: "file-2", sessionKey: sk0, agentId: "agent-0", role: .user,
                    text: "[文件: banner.psd]", timestamp: now.addingTimeInterval(-100), runId: nil)
        fileMsg2.fileSize = 15_700_000
        allMessages.append(fileMsg2)

        var fileMsg3 = ChatMessage(id: "file-3", sessionKey: sk0, agentId: "agent-0", role: .user,
                    text: "[文件: test_data.json]", timestamp: now.addingTimeInterval(-80), runId: nil)
        fileMsg3.fileSize = 4_200
        fileMsg3.localImageData = """
        {
          "users": [
        ...
        """.data(using: .utf8)
        allMessages.append(fileMsg3)

        allMessages.append(ChatMessage(id: "file-3-reply", sessionKey: sk0, agentId: "agent-0", role: .agent,
                    text: "收到「文件: test_data.json」。小南已处理完毕，一切正常。",
                    timestamp: now.addingTimeInterval(-70), runId: nil))

        var fileMsg4 = ChatMessage(id: "file-4", sessionKey: sk0, agentId: "agent-0", role: .agent,
                    text: "[文件: migration_20260328.sql]", timestamp: now.addingTimeInterval(-50), runId: nil)
        fileMsg4.fileSize = 890
        fileMsg4.localImageData = """
        -- Migration: add kanban tables
        CREATE TABLE boards (
        ...
        """.data(using: .utf8)
        allMessages.append(fileMsg4)

        var fileMsg5 = ChatMessage(id: "file-5", sessionKey: sk2, agentId: "agent-2", role: .agent,
                    text: "[文件: 性能报告.xlsx]", timestamp: now.addingTimeInterval(-180), runId: nil)
        fileMsg5.fileSize = 128_500
        allMessages.append(fileMsg5)

        var fileMsg6 = ChatMessage(id: "file-6", sessionKey: sk0, agentId: "agent-0", role: .user,
                    text: "[文件: README.md]", timestamp: now.addingTimeInterval(-30), runId: nil)
        fileMsg6.fileSize = 2_100
        fileMsg6.localImageData = """
        # ClawMo - OpenClaw Gateway Client

        iOS native client for interacting with AI agents.
